<script setup lang="ts">
  ...
  const selectedFlag = computed<RiskFlag | ''>(() => {
    return formData.value.selectedFlag
  })
  ...
  watch(
    ...
    ([newFlag, newName]) => {
      ...
      formData.value.flag = parts.join('_')
  ...
  watch(flagFieldsResponse, response => {
      if (!response) {
        return
      }

      const firstField = response.fields[0]
      const firstConditionType =
        response.condition_types[0]
  ...
  const handleValueFieldInput = (
    ...
    const type = getInputTypeForField(condition.field)

    if (type === 'number') {
      condition.value_field = input.value.replace(
        /[^0-9.-]/g,
        ''
      )
    } else if (type === 'text') {
  ...
  watch(() => formData.value.conditions.map(condition => condition.field), fields => {
    fields.forEach(
      (fieldName, index) => {
        const condition =
          formData.value.conditions[index]
  ...
  const removeCondition = (index: number) => {
    ...
    formData.value.conditions.splice(
      index,
      1
    )
  }
  ...
  const handleAdd = async () => {
    ...
    const payload: RegisterRiskRulePayload = {
      ...
      flags: [formData.value.selectedFlag],
...
<template>
  <div class="modal" :class="{ 'is-active': isOpen }">
    <!-- Backdrop -->
    <div class="modal-background custom-backdrop" @click="close"/>
    ...
    <div class="modal-card custom-modal-card">
      ...
      <header class="modal-card-head custom-header">
        ...
        <button class="delete custom-close" aria-label="close" @click="close"/>
      </header>
      ...
      <section class="modal-card-body custom-body">
        ...
        <div class="columns is-multiline mb-4">
          ...
          <div class="column is-6 pb-0">
            <div class="field">
              ...
              <div class="control">
                <div class="select custom-select is-fullwidth">
                  <select
                    v-model="formData.selectedFlag"
                  ...
                  >
                    ...
                    <option v-for="flag in availableFlags" :key="flag" :value="flag">
                      {{ flag }}
                    </option>
                  </select>
                </div>
        ...
        <!-- Conditions -->
        <template v-if="formData.selectedFlag">
          <hr class="divider" />
          <!-- Conditions Header -->
          ...
          <div v-else class="conditions-wrapper">
            <div v-for="(condition, index) in formData.conditions" :key="condition.id" class="condition-row p-4 mb-3">
              <!-- Row 1: Type, Field, Operator, Value, & Remove -->
              <div class="columns is-mobile is-multiline is-vcentered custom-columns mb-2">
                ...
                <div class="column is-2-desktop is-5-mobile">
                  ...
                  <div class="select is-fullwidth custom-select">
                    <select v-model="condition.operator" :disabled="!condition.field">
                      ...
                      <option v-for="operator in getOperatorsForField(condition.field)" :key="operator" :value="operator">
                        {{ operator }}
                      </option>
                    </select>
                  </div>
                ...
                <div class="column is-1-desktop is-2-mobile is-flex is-justify-content-flex-end is-align-self-end">
                  <button
                    class="modern-btn btn-danger-icon"
...
<style scoped>
  ...
  .custom-input:focus:not(:disabled), .custom-select select:focus {
  ...
  .custom-close:hover::before, .custom-close:hover::after {
